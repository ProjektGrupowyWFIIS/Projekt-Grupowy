...
include("navbar.php");
?>
<h3 class="text-white text-center mt-3">Pliki</h3>
...
open_database();
$atr = read_table("files.files","","order by folder_id, file_name");
	...
	echo "<th style='color: white'> Folder: </th>";
...
foreach($atr as $row_number => $row)
{
	...
	echo '<th style=\'color: white\'>'.get_folder_name($row['folder_id']).'</th>';
	echo '<th>'.'<a href=db_files_updateFile.php?FileID='.$row["file_id"].'&TempFolderID='.$row["folder_id"].'>Edycja</a>'.'</th>';
	echo "</tr>";
