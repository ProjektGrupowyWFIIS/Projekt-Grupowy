...
<head>
  <meta charset="utf-8">
  <title>Wartości atrybutów dla zasobów energetycznych</title>
...
include("navbar.php");
?>
<h3 class="text-white text-center mt-3">Wartości atrybutów (cech nienumerycznych) dla zasobów energetycznych (nośników energii)</h3>
...
open_database();
$atr = read_table("energy_resources.resources_attributes","","order by resource_id,attribute_id");
...
foreach($atr as $row_number => $row)
{
	...
	echo '<th>'.'<a href=db_energy_resources_updateEnergyResourceAttribute.php?ResourceID='.$row["resource_id"].'&AttributeID='.$row["attribute_id"].'>Edycja</a>'.'</th>';
	echo "</tr>";
