...
{
    open_database();
        $query1 = "select * from units.quantities where quantity_id=2;";
        $result1 = pg_query($query1);

        $query2 = "select * from units.units where unit_id = 5;";
                                       ...
                                       $unit_full_name_pl, $unit_full_name_eng)
{
    ...
    $query2 = "update units.units set 
                ...
                '".$unit_full_name_eng."', ".$quantity_id.")  where unit_id = 5;";
...
{
    open_database();
        $query = "select * from units.source_unit_names where unit_variant = 'www';";
        $result = pg_query($query);
    close_database();
    $row = pg_fetch_array($result);
...
function update_source_unit_name($unit_variant, $unit_canonical_id)
{

    $query = "update units.source_unit_names set 
              (unit_variant, unit_canonical_id)= ('".$unit_variant."', '".$unit_canonical_id."')
              where unit_variant = 'www';";
...
function update_hierarchy_of_categories($cat_id, $parent_id)
{
    $query = "update categories.hierarchy_of_categories set 
              (cat_id, parent_id)= ('".$cat_id."', '".$parent_id."')
              where cat_id = 1 and parent_id = 1;";
...
function update_mandatory_attribute($cat_id, $attribute_id)
{
    $query = "update attributes.mandatory_attributes set 
              (cat_id, attribute_id)= ('".$cat_id."', '".$attribute_id."')
              where cat_id = 1 and attribute_id = 3;";
    //echo "DEBUG: ".$query."<br><br>";
...
function update_mandatory_factor($cat_id, $factor_id)
{
    $query = "update factors.mandatory_factors set 
              (cat_id, factor_id)= ('".$cat_id."', '".$factor_id."')
              where cat_id = 4 and factor_id = 'xyz';";
...
function update_resource_category($resource_id, $cat_id)
{
    $query = "update resources.resources_categories set 
              (resource_id, cat_id)= ('".$resource_id."', '".$cat_id."')
              where resource_id = 1 and cat_id = 2;";
